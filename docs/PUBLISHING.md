...
> 本仓库只有**一条发布线**:**26.x** = `2.1.0+mc26.2`(26.1.2 那一个是 `2.0.0+mc26.1.2`,已发布、内容不变)。
...
| 构建配置 | `build.gradle` / `gradle.properties`(仓库根目录) | 版本号 `2.1.0+mc26.2`,产物名 `OptiFabric-Reforged-2.1.0+mc26.2.jar` |
...
| 移植记录 | `docs/PORT_26.x.md` | 26.x 为什么单独一条线、官方名带来的每一类冲突、26.2 那四处 |
| 更新日志 | `CHANGELOG.md` | 26.x 的 2.1.0 / 2.0.0 / 1.2.0 三节,后面是 1.21.x / 1.20.6 的历史 |
...
Copy-Item "build\libs\OptiFabric-Reforged-2.1.0+mc26.2.jar" dist -Force
```

> 这一线**没有 `-Pmc`**:`minecraft_version` 就是 `26.2`、`mod_version_base` 就是 `2.1.0`(都在根目录
...
- `OptiFabric-Reforged-2.1.0+mc26.2.jar` ← **上传这个**
- `OptiFabric-Reforged-2.1.0+mc26.2-sources.jar`(可选,一般不用发)
...
cd I:\mods\OptiFabric
git add -A
git commit -m "OptiFabric Reforged 2.1.0+mc26.2: OptiFine on Fabric for 26.2"
...
把 `OptiFabric-Reforged-2.1.0+mc26.2.jar`
...
2. **上传文件**:一个 jar **一个文件**,版本名用 `2.1.0+mc26.2`,并在文件设置里把 **26.2** 勾上。
...
1. 改根目录 `gradle.properties` 里的 `mod_version_base`(例如 `2.1.1`)—— 别手改,走
   `.\release\version.ps1 -Line 26.x -Kind patch`(见 [`VERSIONING.md`](VERSIONING.md) 第四节);
2. 在 `CHANGELOG.md` 顶部加一节(写清新的版本号,例如 `2.2.0+mc26.2`;`release/notes/mc26.2.md` 同步);
